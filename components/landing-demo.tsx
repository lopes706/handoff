...
export function LandingDemo() {
  ...
  return <div className="parcel hero-visual" aria-describedby={`${previewNoteId} ${instructionsId}`} aria-labelledby={previewTitleId} role="region"><div className="label"><div className="label-head"><div><span className="eyebrow" aria-hidden="true">Local preview · not live</span><h3 id={previewTitleId} style={{ margin: "5px 0 0" }}>Deal #0042</h3></div><span className="status-tape">{active === 0 ? "open" : active === 1 ? "funded" : "released"}</span></div><div className="barcode" aria-hidden="true" /><p className="sr-only" id={previewNoteId}>Local preview only. This demo is not connected to live contract data and cannot submit a real handoff.</p><p className="sr-only" id={instructionsId}>Deal flow preview. Use Left and Right Arrow to move between steps, or Home and End to jump to the first or last step.</p><div aria-atomic="true" aria-labelledby={`demo-step-tab-${active}`} aria-live="polite" id={panelId} role="tabpanel" style={{ display: "flex", gap: 14, alignItems: "center", margin: "17px 0" }}><Icon aria-hidden="true" /><div><strong>{step.title}</strong><p className="fineprint" style={{ margin: 0 }}>{step.copy}</p></div></div><div aria-describedby={instructionsId} aria-keyshortcuts="ArrowLeft ArrowRight Home End" aria-label="Deal flow preview steps" aria-orientation="horizontal" className="demo-steps" role="tablist">{steps.map((item, index) => <button key={item.title} aria-controls={panelId} aria-selected={index === active} data-state={index < active ? "complete" : index === active ? "current" : "upcoming"} id={`demo-step-tab-${index}`} onClick={() => setActive(index)} onKeyDown={(event) => {
  ...
  }} className="demo-step" role="tab" style={{ width: "100%", borderInline: 0, borderTop: 0, textAlign: "left" }} tabIndex={index === active ? 0 : -1} type="button"><b>{index + 1}</b><span><strong>{item.title}</strong><small style={{ display: "block" }}>{index < active ? "Complete" : index === active ? "Current stop" : "Waiting"}</small></span></button>)}</div></div></div>;
}
