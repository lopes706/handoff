...
export function ReleaseScanner({ onRead }: { onRead(value: string): void }) {
  ...
  return <div>{error ? <div className="scan-frame"><CameraOff size={42} /><span className="sr-only">Camera unavailable</span></div> : <div className="scan-frame"><video ref={video} className="scanner-video" muted playsInline aria-label="Release pass camera scanner" /><ScanLine className="sr-only" /></div>}{error && <p role="status" className="warning"><CameraOff />{error}</p>}</div>;
}
