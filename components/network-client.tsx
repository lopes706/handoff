...
import type { HandoffRepository, Network } from "@/lib/types";
...
export function useNetworkClient(network: Network): { account: string; connected: boolean; connecting: boolean; isMiniPay: boolean; repository: HandoffRepository; connect(): Promise<string>; disconnect(): Promise<void> } {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      if (network === "celo") {
        const nextProvider = injectedProvider(); if (!nextProvider) throw new Error("Open Handoff in MiniPay or install a Celo-compatible wallet.");
        const accounts = await nextProvider.request({ method: "eth_requestAccounts" }) as string[]; const active = Number.parseInt(await nextProvider.request({ method: "eth_chainId" }) as string, 16);
        if (active !== getCeloChainId() && !nextProvider.isMiniPay) { try { await nextProvider.request({ method: "wallet_switchEthereumChain", params: [{ chainId: `0x${getCeloChainId().toString(16)}` }] }); } catch { await nextProvider.request({ method: "wallet_addEthereumChain", params: [{ chainId: `0x${getCeloChainId().toString(16)}`, chainName: getCeloChainId() === 42220 ? "Celo" : "Celo Sepolia", nativeCurrency: { name: "CELO", symbol: "CELO", decimals: 18 }, rpcUrls: [getCeloRpc()], blockExplorerUrls: [getCeloExplorer()] }] }); } }
