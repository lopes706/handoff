...
const celoDefaults = deployments.celo[celoNetwork];
const stacksDefaults = deployments.stacks[stacksNetwork];
...
export const publicEnv = {
  ...
  celoNetwork,
  celoContractAddress: read(process.env.NEXT_PUBLIC_HANDOFF_CELO_CONTRACT_ADDRESS, celoDefaults.contractAddress) as Address | "",
  celoDeploymentBlock: read(process.env.NEXT_PUBLIC_HANDOFF_CELO_DEPLOYMENT_BLOCK, celoDefaults.deploymentBlock),
  celoUsdtAddress: read(process.env.NEXT_PUBLIC_HANDOFF_CELO_USDT_ADDRESS, celoDefaults.usdtAddress) as Address,
  ...
  stacksNetwork,
  stacksContractAddress: read(process.env.NEXT_PUBLIC_HANDOFF_STACKS_CONTRACT_ADDRESS, stacksDefaults.contractAddress),
  stacksContractName: read(process.env.NEXT_PUBLIC_HANDOFF_STACKS_CONTRACT_NAME, stacksDefaults.contractName),
  stacksSbtcContractId: read(process.env.NEXT_PUBLIC_STACKS_SBTC_CONTRACT_ID, stacksDefaults.sbtcContractId),
...
export const getCeloChainId = () => deployments.celo[publicEnv.celoNetwork].chainId;
...
export const contractId = (network: Network) => network === "celo" ? publicEnv.celoContractAddress : publicEnv.stacksContractAddress ? `${publicEnv.stacksContractAddress}.${publicEnv.stacksContractName}` : "";
