...
Human-readable terms never enter a Handoff database: they live in a checksummed URL fragment or `.handoff.json` file, and only their SHA-256 hash is onchain. The fragment is omitted from HTTP requests, which reduces server exposure, but anyone with the deal sheet link or portable file can still read it because neither is encrypted.
...
`refunded`, `cancelled`, or `expired` to preview another state. Use `funded-buyer` or
