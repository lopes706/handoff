...
Human-readable terms never enter a Handoff database: they live in a checksummed URL fragment or `.handoff.json` file, and only their SHA-256 hash is onchain. The fragment is omitted from HTTP requests, but anyone with the deal sheet link can still read it because it is not encrypted.
